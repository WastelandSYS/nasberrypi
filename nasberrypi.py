...
DEVICE = os.environ.get(
    "NASBERRY_DEVICE",
    "/dev/disk/by-label/NasberryDRV"
)

MOUNT_POINT = os.environ.get(
    "NASBERRY_MOUNT_POINT",
    "/mnt/nasberry"
)

GUI_MOUNT = os.environ.get(
    "NASBERRY_GUI_MOUNT",
    "/media/kali/NasberryDRV"
)
...
CHECK_DELAY = int(
    os.environ.get(
        "NASBERRY_CHECK_DELAY",
        "2"
    )
)

SAMBA_SERVICE = os.environ.get(
    "NASBERRY_SAMBA_SERVICE",
    "smbd"
)

SAMBA_SERVICES = os.environ.get(
    "NASBERRY_SAMBA_SERVICES",
    "smbd,nmbd,winbind"
).split(",")
...
def cleanup_gui_mount():
    if os.path.ismount(GUI_MOUNT):
        ...
        result = run(
            sudo_cmd("umount", GUI_MOUNT)
        )

        if result.returncode != 0:
            log(
                f"✖ Could not unmount GUI mount: "
...
def service_exists(service):
    ...
    return (
        result.returncode == 0 and
        f"{service}.service" in result.stdout
    )
...
def enforce_boot_safety():
    ...
    for service in SAMBA_SERVICES:
        ...
        if enabled.stdout.strip() == "enabled":
            ...
            log(
                f"{service} is enabled at boot "
                f"→ disabling for safe mode"
            )
...
def start_share():
    ...
    if not is_mounted():
        ...
        log(
            ...
            "— mounting before share start"
        )

        if not mount_storage():

            log(
                "✖ Refusing to start NAS share "
...
def verify_pin():
    ...
    if PIN_CODE == "1234":
        log(
            ...
            "Set NASBERRY_PIN before real NAS use."
        )
...
def panic_lock():
    ...
    if share_stopped and storage_unmounted:
        ...
        log(
            "NAS LOCKED DOWN "
            "(share stopped + storage unmounted)"
        )

        return True

    log(
        "NAS LOCKDOWN INCOMPLETE "
...
def write_state(mounted, shared):
    ...
    with open(STATE_FILE, "w") as f:
        ...
        f.write(
            f"MOUNTED={1 if mounted else 0}\n"
        )

        f.write(
            f"SHARED={1 if shared else 0}\n"
        )
...
def status():
    ...
    print(f"""
           ...
           ║                NAS STATUS                  ║
           ╠════════════════════════════════════════════╣
           ║ Storage Mounted : {str(is_mounted()):<25}║
           ║ Share Active    : {str(service_active()):<25}║
           ║ Device Present  : {str(device_exists()):<25}║
           ║ Disk Space      : {disk_usage():<25}║
           ║ Mount Point     : {MOUNT_POINT:<25}║
           ╚════════════════════════════════════════════╝
""")
